...
def main(filename,shadingrate=10,pixelvar=0.1,
  ...
  ri.Begin(filename)
  ri.Option('searchpath', {'string archive':'../../Lecture3Lighting/assets/:./assets/:@'})
  ...
  ri.Pattern('band','band', { 'string direction' : ["horizontal"],
                              'float begin' : [0], 'float end' :[1],
                              'color C1' : [0,0,1], 'color C2' : [0,1,0]

   })

  ri.Bxdf( 'PxrDisney','bxdf', { 'reference color baseColor' : ['band:Cout'] })
  ri.TransformBegin()
