...
def Scene(ri) :
	...
	while (plank <=5.0) :
		...
		ri.Surface(  "WardAnistropic" ,{"float Ka": [0.500],"float Kd": [1.000], "float xroughness": [0.100],"float yroughness": [0.800],"float Ks": [0.800], })
	...
	ri.TransformBegin()
	ri.AttributeBegin()
	ri.Color([0.4,0.4,0.4])
	ri.Translate( 0,-1.0,0)
	ri.Rotate(-90,1,0,0)
	ri.Rotate(36,0,0,1)
	ri.Scale(0.4,0.4,0.4)
	ri.Surface(  "WardAnistropic" ,{"float Ka": [0.500],"float Kd": [1.000], "float xroughness": [0.20],"float yroughness": [0.2],"float Ks": [0.700], })
	ri.Geometry("teapot")
	ri.AttributeEnd()
	ri.TransformEnd()
