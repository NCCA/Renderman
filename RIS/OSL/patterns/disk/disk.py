...
ri.AttributeEnd()
ri.Pattern("disk", shaderName, {"color baseColour": [1.0, 0.2, 0.0], "float repeatU": [4], "float repeatV": [2]})
# second teapot
ri.Bxdf("PxrDisney", "bxdf", {"reference color baseColor": ["%s:Cout" % shaderName]})
drawTeapot(ri, ry=-45)
ri.Pattern(
    "disk", shaderName, {"float repeatU": [8], "float repeatV": [8], "color spotColour": [0, 0, 1], "float fuzz": [0.8]}
)
# third teapot
ri.Bxdf("PxrDisney", "bxdf", {"reference color baseColor": ["%s:Cout" % shaderName]})
drawTeapot(ri, x=1, ry=-45)
