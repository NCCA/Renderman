...
#######################################################################
# first teapot
ri.Pattern("PxrTexture", "logo", {"string filename": "nccatransp.tx", "int invertT": [0]})
ri.Pattern(
    "PxrMix",
    "mixer",
    {
        "color color1": [0.2, 0.5, 0.8],
        "reference color color2": ["logo:resultRGB"],
        "reference float mix": ["logo:resultA"],
    },
)

ri.Bxdf("PxrDisney", "bxdf", {"reference color baseColor": ["mixer:resultRGB"]})
...
ri.Displace(
    "PxrDisplace",
    "displaceTexture",
    {"reference float dispScalar": ["logo:resultA"], "uniform float dispAmount": [0.02], "int enabled": [1]},
)
...
ri.Pattern(
    "PxrTexture",
    "logorepeat",
    {"string filename": "nccatransp.tx", "int invertT": [0], "reference struct manifold": ["tileManifold:result"]},
)
ri.Pattern(
    "PxrMix",
    "mixerrepeat",
...
ri.Bxdf("PxrDisney", "bxdf", {"reference color baseColor": ["mixerrepeat:resultRGB"]})
ri.Displace(
    "PxrDisplace",
    "displaceTexture",
    {"reference float dispScalar": ["logorepeat:resultA"], "uniform float dispAmount": [0.02], "int enabled": [1]},
)
