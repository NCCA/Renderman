...
ri.Option(
    ...
    {
        "string procedural": [
            ".:${RMANTREE}/etc:${RMANTREE}/lib/plugins:/Applications/Pixar/RenderManProServer-22.3/lib/plugins"
        ]
    },
)
...
ri.ShadingRate(0.5)
ri.Integrator(
    "PxrPathTracer", "integrator", {"int numLightSamples": [4], "int numBxdfSamples": [4], "int maxPathLength": [1]}
)
...
ri.Light("PxrDomeLight", "domeLight", {"float exposure": [1.0], "string lightColorMap": ["../usingzip/studio2.tx"]})
ri.AttributeEnd()
...
ri.Volume(
    "blobbydso:impl_openvdb.so",
    [-100, 100, -100, 100, -100, 100],
    [0, 0, 0],
    {"constant string[2] blobbydso:stringargs": ["teapot.vdb", "density"], "varying float density": []},
)
